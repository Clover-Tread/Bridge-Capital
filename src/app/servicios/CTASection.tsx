...
const CTASection = () => {
  const ctaButtonBaseColor = "bg-[#79242F]"; // Tu color #79242F
  const ctaButtonHoverColor = "hover:bg-[#601e26]"; // Un tono ligeramente más oscuro de #79242F

  return (
    <section className="py-16 md:py-24 bg-(--color-light-grey) w-full"> {/* Fondo claro para la sección */}
      <div className={`mx-auto ${SHARED_MAX_WIDTH_CLASS} ${SHARED_HORIZONTAL_PADDING_CLASSES} text-center`}>
        <h2 className="text-3xl md:text-4xl font-[800] text-(--color-primary) mb-6 md:mb-10 leading-tight">
          ¿Tienes Dudas? <br className="block sm:hidden" /> Contáctanos para Asesorarte.
        </h2>
        <Button 
          asChild // Para que el botón funcione como un Link de Next.js
          className={`
            ${ctaButtonBaseColor} 
            ${ctaButtonHoverColor}
            text-white 
          ...
          `}
        >
          <Link href="/contacto"> {/* Enlaza a tu página de contacto */}
            Contáctanos
          </Link>
        </Button>
      </div>
