    ...
    <section className="w-full flex flex-col lg:flex-row items-center gap-8 lg:gap-12">
      ...
      <div className={`w-full lg:w-1/2 ${imagePosition === 'right' ? 'lg:order-2' : 'lg:order-1'}`}>
        <Card className="group relative w-full overflow-hidden rounded-xl shadow-lg 
              ...
              // Ajusta estas alturas responsivas según tus necesidades para la forma rectangular
        >
          <CardContent className="p-0 w-full h-full relative"> {/* p-0, w-full, h-full para que Image llene CardContent */}
            <Image
              src={imageUrl}
              alt={imageAlt}
              layout="fill"
      ...
      {/* Columna de Texto */}
      <div className={`w-full lg:w-1/2 space-y-4 ${imagePosition === 'right' ? 'lg:order-1' : 'lg:order-2'}`}>
        <h2 className="text-5xl md:text-6xl font-[900] text-[var(--color-primary)] text-center"> {/* Usa tu variable de color primario */}
          {title}
        </h2>
